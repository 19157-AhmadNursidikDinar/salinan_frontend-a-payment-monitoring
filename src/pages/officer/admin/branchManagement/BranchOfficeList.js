...
export default function BranchOfficeList(props) {
      ...
      <Paper className={classes.PaperSize} elevation={4}>
        ...
        >
          <Table className={classes.table} aria-label="custom table pagination">
            ...
            <TableBody>
              {(rowsPage > 0
                ? branchs.slice(pages * rowsPage, pages * rowsPage + rowsPage)
                : branchs
              ).map((branch, index) => (
                <StylingTableRow key={index + 1}>
                  <StylingTableCell width="10%">
                    {pages * rowsPage + index + 1}
                  </StylingTableCell>
                  <StylingTableCell>
                    {branch.id === 1 ? "pusat" : branch.branch_name}
                  </StylingTableCell>
                </StylingTableRow>
              ))}
            </TableBody>
  ...
  return (
    <ContentContainer role="admin" selectedMenu="Daftar Kantor Cabang">
      ...
      >
        <Typography variant="h4">Daftar Kantor Cabang</Typography>
      </div>
      ...
      {isLoading ? <TableSkeleton /> : <PaperListBranch />}
    </ContentContainer>
  );
}
