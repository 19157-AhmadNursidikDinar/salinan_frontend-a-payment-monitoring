...
export function FormAddService(props) {
    ...
    const { dataBranch, id, recomendation, capacity } = props
    ...
    const formik = useFormik({
        ...
        onSubmit: async ({ branch_id, capacity }) => {
            const result = await SlaService.updateSLA({ branch_id, capacity });
            if (!Boolean(result.error)) {
                props.history.push("/service-level-agreement", { success: true, message: "Service level agreement telah diupdate" });
            } else{
                console.log("error");
            }
        }
...
export default function UpdateServiceLevelAgreement(props) {

    const [dataBranch, setDataBranch] = useState([]);
    const { id } = useParams();
    const { recomendation, capacity } = props.location.state
    ...
    return (
        <ContentContainer role="admin" selectedMenu="Service Lvl Agreement">
            ...
            >
                <Typography variant="h4">Update Service Level Agreement</Typography>
            </div>
            <FormAddService history={props.history} dataBranch={dataBranch} id={id} recomendation={recomendation} capacity={capacity} />
        </ContentContainer>
    )
}
